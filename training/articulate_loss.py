...
class MultitaskLoss(torch.nn.Module):
    ...
    def forward(self, predictions, batch) -> torch.Tensor:
        ...
        total_loss = 0
        loss_dict = {}
        nonpad_masks = batch["nonpad_masks"][:, 1:] # [2026-02-12] @tcm: skip first view
        ...
        gt_joint_types = batch["joint_types"][:, 1:]
        ...
        gt_joint_axis = batch["joint_axes"][:, 1:, :] # [2026-02-11] @tcm: skip first view
        ...
        assert joint_axis.shape == gt_joint_axis.shape, f'unmatched shape: {joint_axis.shape} != {gt_joint_axis.shape}'
        ...
        gt_joint_origin: torch.Tensor = batch["joint_origins"][:, 1:, :] # [2026-02-11] @tcm: skip first view
        assert joint_origin.shape == gt_joint_origin.shape, f'unmatched shape: {joint_origin.shape} != {gt_joint_origin.shape}'
        diff_norms = torch.linalg.vector_norm(joint_origin - gt_joint_origin, ord=2, dim=-1) + REAL_EPS
        revolute_origin_masks = batch["revolute_origin_masks"][:, 1:]  # [2026-02-11] @tcm: skip first view
        origin_loss = (diff_norms * nonpad_masks * revolute_origin_masks)
        origin_loss = (origin_loss.sum(dim=-1) / (revolute_origin_masks.sum(dim=-1) + REAL_EPS)).mean()
        ...
        seg_gt = batch["seg_images"][:, 1:, ...] # [2026-02-12] @tcm: skip first view
        seg_loss: torch.Tensor = F.cross_entropy(seg_pred, seg_gt) # [2026-02-06] @tcm: average over all loss elements
